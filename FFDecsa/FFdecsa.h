...
// how many).
__declspec(dllexport) int get_internal_parallelism(void);
...
// the list).
__declspec(dllexport) int get_suggested_cluster_size(void);

// -- alloc & free the key structure
__declspec(dllexport) void *get_key_struct(void);
__declspec(dllexport) void free_key_struct(void *keys);

// -- set control words, 8 bytes each
__declspec(dllexport) void set_control_words(void *keys, const unsigned char *even, const unsigned char *odd);

// -- set even control word, 8 bytes
__declspec(dllexport) void set_even_control_word(void *keys, const unsigned char *even);

// -- set odd control word, 8 bytes
__declspec(dllexport) void set_odd_control_word(void *keys, const unsigned char *odd);
...
__declspec(dllexport) int decrypt_packets(void *keys, unsigned char **cluster);
